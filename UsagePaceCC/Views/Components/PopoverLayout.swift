...
enum PopoverLayout {
    ...
    static func rowCount(
        ...
        antigravityErrorMessage: String? = nil
    ) -> Int {
        ...
        var rows = items.count + legacyRows
        if codexErrorMessage != nil {
            rows += 1
        }
        // Antigravity provider-level error row (N-account rule, deliberately NOT Codex's per-provider
        // replacement rule): an account that failed AFTER previously succeeding keeps rendering its
        // last-known window data (same as Claude) — it does not disappear, and while
        // at least one Antigravity account is still healthy this fallback row must NOT fire (the
        // healthy account's row is still on screen and correct). It DOES need to fire once EVERY
        // known Antigravity account currently carries an error — including ones that failed after a
        // prior success and are still rendering stale cached data — otherwise a single permanently
        // failing account (single-account user, revoked OAuth grant) shows week-old numbers forever
        // with no indicator at all. `items.contains` from `legendItems` cannot
        // detect this: a stale-but-cached account always produces a row. So this checks
        // `antigravitySnapshots` directly instead.
        // - Important: this `||` expression must stay character-identical to the one in
        //   `UsageDetailView.antigravityErrorRow` (Locked Constraint: row count vs. actual render).
        if antigravityErrorMessage != nil,
    ...
    // MARK: - Account-driven legend rows 
    ...
    private static func isAccountDrivenType(_ type: LimitType) -> Bool {
        type == .fiveHour || type == .sevenDay || type == .codexPrimary || type == .codexSecondary
            || type == .antigravityPrimary || type == .antigravitySecondary
    }
    ...
    /// Single source of truth for the new account-driven legend rows : 2 rows per saved Claude
    ...
    static func legendItems(
        ...
        codexAccount: Account? = nil,
        antigravitySnapshots: [AccountUsageSnapshot] = [],
        activeDisplayTypes: [LimitType] = [.fiveHour, .sevenDay, .codexPrimary, .codexSecondary, .antigravityPrimary, .antigravitySecondary]
    ) -> [LegendRowItem] {
        ...
        // Antigravity : a *collection*, so this is a byte-for-byte analogue of the Claude loop
        // above, not the Codex single-value block — Antigravity's OAuth source is multi-account.
        // Provider order stays locked: Claude → Codex → Antigravity.
        for snapshot in antigravitySnapshots {
            if snapshot.fiveHour != nil, activeDisplayTypes.contains(.antigravityPrimary) {
                items.append(LegendRowItem(accountId: snapshot.accountId, snapshot: snapshot, window: .fiveHour))
            }
            if snapshot.sevenDay != nil, activeDisplayTypes.contains(.antigravitySecondary) {
                items.append(LegendRowItem(accountId: snapshot.accountId, snapshot: snapshot, window: .sevenDay))
            }
        }
