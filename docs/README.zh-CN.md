...
**在菜单栏中优雅地追踪您的 Claude（以及可选的 Codex）订阅用量。**
...
- **📊 实时监控** - 在菜单栏实时显示 Claude 订阅（Free/Pro/Team/Max）的使用配额
...
- **👥 多账户管理** - 支持多账户 / 同一账户多组织，快速切换
- **✨ Codex 支持** - 可选的 Codex 用量监控，与 Claude 并列显示为双栏视图（在设置中添加 Codex 账户即可启用）
- **🌐 内置浏览器登录** - 内置浏览器登录自动提取 Session Key，无需手动复制
...
**认证信息** - 多账户管理（添加/删除/切换/别名编辑）、内置浏览器登录、手动输入、连接诊断
**关于** - 版本信息和相关链接

### 欢迎界面

**配置认证信息** - 支持内置浏览器一键登录（推荐）或手动输入 Session Key，自动获取 Organization ID，支持同一 Session Key 下的多组织自动创建
...
- **默认显示** - 任务栏图标显示使用量百分比
- **查看详情** - 点击任务栏图标即可查看详情
- **手动刷新** - 详情窗口点击刷新按钮或使用快捷键 ⌘R（打开主界面时也会自动刷新数据）
- **切换账户** - 在详情窗口点击「…」菜单或右键点击任务栏图标，选择要切换的账户
- **快捷键操作**
  - ⌘R - 手动刷新数据
  - ⌘, - 打开通用设置
  - ⌘⇧A - 打开认证设置
...
1. 打开设置 → 认证信息
2. 点击「浏览器登录」重新登录（推荐），或按照手动方式重新获取 Session Key
3. 完成后即可恢复正常
...
A: Usage4Claude 支持多账户和同一账户下的多组织管理：
- **添加账户** - 在设置 → 认证信息中点击添加按钮，通过浏览器登录或手动输入
- **切换账户** - 在详情窗口点击「…」菜单或右键点击任务栏图标，选择要切换的账户
- **编辑别名** - 为每个账户设置易于辨识的别名
...
A: 在设置 → 通用设置中可以开关用量通知功能：
- **用量警告** - 当使用量达到 90% 时发送系统通知
- **重置通知** - 当配额重置时发送通知提醒
