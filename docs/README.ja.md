...
- **📊 リアルタイム監視** - Claude サブスクリプション（Free/Pro/Team/Max）の使用クォータをメニューバーに表示
...
- **🔔 使用量通知** - 使用量警告とリセット通知をサポート、設定でオン/オフ可能
...
**認証情報** - マルチアカウント管理（追加/削除/切り替え/エイリアス編集）、内蔵ブラウザログイン、手動入力、接続診断
**について** - バージョン情報と関連リンク

### ウェルカム画面

**認証情報の設定** - 内蔵ブラウザワンクリックログイン（推奨）または Session Key の手動入力、Organization ID を自動取得、同一 Session Key 下の複数組織を自動作成
...
- **詳細を表示** - メニューバーアイコンをクリックして詳細を表示
- **手動リフレッシュ** - 詳細ウィンドウのリフレッシュボタンをクリック、またはショートカット ⌘R を使用（メインウィンドウを開くとデータも自動更新）
- **アカウント切り替え** - 詳細ウィンドウの「…」メニューまたはメニューバーアイコンを右クリックして、切り替えるアカウントを選択
- **キーボードショートカット**
  - ⌘R - データを手動でリフレッシュ
  - ⌘, - 一般設定を開く
  - ⌘⇧A - 認証設定を開く
...
1. 設定 → 認証情報を開く
2. 「ブラウザログイン」をクリックして再ログイン（推奨）、または手動で新しい Session Key を取得
3. 完了後、監視が再開されます
...
A: Usage4Claude はマルチアカウントと同一アカウント下の複数組織管理をサポートしています：
- **アカウント追加** - 設定 → 認証情報で追加ボタンをクリック、ブラウザログインまたは手動入力で追加
- **アカウント切り替え** - 詳細ウィンドウの「…」メニューまたはメニューバーアイコンを右クリックして、切り替えるアカウントを選択
...
A: 設定 → 一般で使用量通知機能のオン/オフを切り替えられます：
- **使用量警告** - 使用率が90%に達するとシステム通知を送信
- **リセット通知** - クォータがリセットされると通知を送信
