...
**메뉴 바에서 Claude(및 선택적 Codex) 구독 할당량을 아름답게 추적하세요.**
...
- **📊 실시간 모니터링** - 메뉴 바에서 Claude 구독(Free/Pro/Team/Max) 사용 할당량을 실시간 표시
...
- **👥 다중 계정 관리** - 다중 계정 / 동일 계정 다중 조직 지원, 빠른 전환
- **✨ Codex 지원** - 선택적 Codex 사용량 모니터링, Claude와 나란히 이중 열 뷰로 표시(설정에서 Codex 계정 추가 시 활성화)
- **🌐 내장 브라우저 로그인** - 내장 브라우저 로그인으로 Session Key 자동 추출, 수동 복사 불필요
...
- **🔔 사용량 알림** - 사용량 경고 및 리셋 알림 지원, 설정에서 켜기/끄기 가능
...
**인증** - 다중 계정 관리(추가/삭제/전환/별명 편집), 내장 브라우저 로그인, 수동 입력, 연결 진단
**정보** - 버전 정보 및 관련 링크

### 환영 화면

**인증 정보 구성** - 내장 브라우저 원클릭 로그인(권장) 또는 Session Key 수동 입력, Organization ID 자동 검색, 동일 Session Key 하 다중 조직 자동 생성
...
- **상세 보기** - 메뉴 바 아이콘을 클릭하여 상세 정보 확인
- **수동 새로고침** - 상세 창에서 새로고침 버튼 클릭 또는 단축키 ⌘R 사용(메인 창 열 때 데이터 자동 새로고침)
- **계정 전환** - 상세 창에서 "…" 메뉴 또는 메뉴 바 아이콘 우클릭으로 전환할 계정 선택
- **키보드 단축키**
  - ⌘R - 수동으로 데이터 새로고침
  - ⌘, - 일반 설정 열기
  - ⌘⇧A - 인증 설정 열기
...
1. 설정 → 인증 열기
2. "브라우저 로그인"을 클릭하여 다시 로그인(권장), 또는 수동으로 새 Session Key 가져오기
3. 완료 후 모니터링이 재개됩니다
...
A: Usage4Claude는 다중 계정 및 동일 계정 하 다중 조직 관리를 지원합니다:
- **계정 추가** - 설정 → 인증에서 추가 버튼 클릭, 브라우저 로그인 또는 수동 입력으로 추가
- **계정 전환** - 상세 창에서 "…" 메뉴 또는 메뉴 바 아이콘 우클릭으로 전환할 계정 선택
...
A: 설정 → 일반에서 사용량 알림 기능을 켜거나 끌 수 있습니다:
- **사용량 경고** - 사용량이 90%에 도달하면 시스템 알림 전송
- **리셋 알림** - 할당량이 재설정되면 알림 전송
