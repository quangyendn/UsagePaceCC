...
- **📊 即時監控** - 在選單列即時顯示 Claude 訂閱（Free/Pro/Team/Max）的使用配額
...
**認證資訊** - 多帳戶管理（新增/刪除/切換/別名編輯）、內建瀏覽器登入、手動輸入、連線診斷
**關於** - 版本資訊和相關連結

### 歡迎畫面

**設定認證資訊** - 支援內建瀏覽器一鍵登入（推薦）或手動輸入 Session Key，自動獲取 Organization ID，支援同一 Session Key 下的多組織自動建立
...
- **預設顯示** - 工作列圖示顯示使用量百分比
- **查看詳情** - 點擊工作列圖示即可查看詳情
- **手動重新整理** - 詳情視窗點擊重新整理按鈕或使用快速鍵 ⌘R（開啟主介面時也會自動重新整理資料）
- **切換帳戶** - 在詳情視窗點擊「…」選單或右鍵點擊工作列圖示，選擇要切換的帳戶
- **鍵盤快速鍵操作**
  - ⌘R - 手動重新整理資料
  - ⌘, - 開啟一般設定
  - ⌘⇧A - 開啟認證設定
...
1. 開啟設定 → 認證資訊
2. 點擊「瀏覽器登入」重新登入（推薦），或按照手動方式重新取得 Session Key
3. 完成後即可恢復正常
...
A: Usage4Claude 支援多帳戶和同一帳戶下的多組織管理：
- **新增帳戶** - 在設定 → 認證資訊中點擊新增按鈕，透過瀏覽器登入或手動輸入
- **切換帳戶** - 在詳情視窗點擊「…」選單或右鍵點擊工作列圖示，選擇要切換的帳戶
- **編輯別名** - 為每個帳戶設定易於辨識的別名
...
A: 在設定 → 一般設定中可以開關用量通知功能：
- **用量警告** - 當使用量達到 90% 時發送系統通知
- **重置通知** - 當配額重置時發送通知提醒
